...
This matrix connects material claims from the original draft and later reviews
to their evidence, human explanation, and runtime instruction. The human
destinations are indexed in xref:README.adoc[the TDD knowledge-base index]; the
runtime destinations are in
link:../../skills/develop-with-tdd/SKILL.md[the develop-with-tdd skill].

“Research verdict” summarizes the relevant finding in
xref:research-findings.adoc[research-findings.adoc]. Human and skill
destinations must remain aligned when a claim changes.

[cols=",,,",options="header",]
|===
|Claim or decision |Research verdict |Human destination |AI/skill destination
|TDD is primarily a design-feedback discipline |Defensible thesis, not a universal definition |`guide/01-why-tdd.adoc` |`SKILL.md`; operating reference
